...
def perform_statistical_tests(data1, data2):
    ...
    # Test 5: Execution Time - ADD THIS ENTIRE BLOCK
    statistic, p_value = mannwhitneyu(data1['execution_times'],
                                      data2['execution_times'],
                                      alternative='two-sided')
    results.append({
        'Metric': 'Execution Time (s)',
        'Method 1 Mean': np.mean(data1['execution_times']),
        'Method 1 Std': np.std(data1['execution_times']),
        'Method 2 Mean': np.mean(data2['execution_times']),
        'Method 2 Std': np.std(data2['execution_times']),
        'U-statistic': statistic,
        'p-value': p_value,
        'Significant (α=0.05)': 'Yes' if p_value < 0.05 else 'No'
    })
...
def load_method_data(results_dir, method_name):
    ...
    for run_file in run_files:
        ...
        execution_times.append(float(exec_time))  # ADD THIS
    ...
    return {
        ...
        'execution_times': np.array(execution_times)  # ADD THIS
    }
...
def create_visualizations(data1, data2, output_dir='plots'):
    ...
    for metric_key, metric_name, scale_factor in metrics:
        ...
        plt.savefig(os.path.join(output_dir, f'{filename}_comparison_4.png'),
    ...
    with pd.ExcelWriter('statistical_comparison_results_test_4.xlsx', engine='openpyxl') as writer:
        ...
        for method_name, data in [('Basic Method', basic_data), ('Optimal Method', optimal_data)]:
            summary_data.append({
                ...
                'Execution Time Mean (s)': np.mean(data['execution_times']),  # ADD THIS
                'Execution Time Std (s)': np.std(data['execution_times'])  # ADD THIS
            })
